MODULE sbcblk_algo_coare
  ...
  PRIVATE
  PUBLIC :: TURB_COARE
  REAL(KIND = wp), PARAMETER :: zi0 = 600._wp
  REAL(KIND = wp), PARAMETER :: Beta0 = 1.250_wp
  ...
  FUNCTION alfa_charn(pwnd)
    ...
    REAL(KIND = wp), DIMENSION(jpi, jpj), INTENT(IN) :: pwnd
    INTEGER :: ji, jj
    ...
    DO jj = 1, jpj
      DO ji = 1, jpi
        zw = pwnd(ji, jj)
        zgt10 = 0.5 + SIGN(0.5, (zw - 10.))
        zgt18 = 0.5 + SIGN(0.5, (zw - 18.))
        alfa_charn(ji, jj) = (1. - zgt10) * 0.011 + zgt10 * ((1. - zgt18) * (0.011 + (0.018 - 0.011) * (zw - 10.) / (18. - 10.)) + zgt18 * (0.018))
      END DO
    END DO
    !$ACC END KERNELS
