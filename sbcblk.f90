...
902 IF (ios > 0) CALL ctl_nam(ios, 'namsbc_blk in configuration namelist', lwp)
  ...
  SUBROUTINE sbc_blk(kt)
    ...
    CALL ProfileStart('sbc_blk', 'r0', psy_profile0)
  ...
  SUBROUTINE blk_oce(kt, sf, pst, pu, pv)
    ...
    zwnd_j(:, :) = 0._wp
    !$ACC END KERNELS
    CALL ProfileStart('blk_oce', 'r0', psy_profile0)
    DO jj = 2, jpjm1
      DO ji = 2, jpim1
    ...
    zztmp = 1. - albo
    !$ACC END KERNELS
    CALL ProfileStart('blk_oce', 'r1', psy_profile1)
    IF (ln_dm2dc) THEN
    ...
    END DO
    !$ACC END KERNELS
    CALL ProfileStart('blk_oce', 'r2', psy_profile2)
    ...
    IF (ABS(rn_zu - rn_zqt) < 0.01_wp) THEN
      CALL ProfileStart('blk_oce', 'r3', psy_profile3)
    ...
    ELSE
      ...
      !$ACC END KERNELS
      CALL ProfileStart('blk_oce', 'r4', psy_profile4)
      zqsb(:, :) = cp_air(sf(jp_humi) % fnow(:, :, 1)) * zqla(:, :) * Ch_atm(:, :) * (zst(:, :) - t_zu(:, :))
      CALL ProfileEnd(psy_profile4)
    END IF
    CALL ProfileStart('blk_oce', 'r5', psy_profile5)
    ...
    qsr_oce(:, :) = qsr(:, :)
    !$ACC END KERNELS
    CALL ProfileStart('blk_oce', 'r6', psy_profile6)
    IF (nn_ice == 0) THEN
  ...
  FUNCTION L_vap(psst)
    ...
    TYPE(ProfileData), SAVE :: psy_profile0
    CALL ProfileStart('l_vap', 'r0', psy_profile0)
  ...
  SUBROUTINE blk_ice_tau
    ...
    wndm_ice(:, :) = 0._wp
    !$ACC END KERNELS
    CALL ProfileStart('blk_ice_tau', 'r0', psy_profile0)
    DO jj = 2, jpjm1
      DO ji = 2, jpim1
    ...
    vtau_ice(:, :) = 0._wp
    !$ACC END KERNELS
    CALL ProfileStart('blk_ice_tau', 'r3', psy_profile3)
    DO jj = 2, jpjm1
      DO ji = 2, jpim1
  ...
  SUBROUTINE Cdn10_Lupkes2015(Cd, Ch)
    ...
    zst(:, :) = sst_m(:, :) + rt0
    !$ACC END KERNELS
    CALL ProfileStart('cdn10_lupkes2015', 'r0', psy_profile0)
