MODULE restart
  ...
  SUBROUTINE rst_opn(kt)
    ...
    CALL ProfileStart('rst_opn', 'r0', psy_profile0)
    IF (kt == nit000) THEN
      lrst_oce = .FALSE.
      IF (ln_rst_list) THEN
        nrst_lst = 1
  ...
  SUBROUTINE rst_write(kt)
    ...
    INTEGER, INTENT(IN) :: kt
    TYPE(ProfileData), SAVE :: psy_profile0
    CALL ProfileStart('rst_write', 'r0', psy_profile0)
  ...
  SUBROUTINE rst_read_open
    ...
    CHARACTER(LEN = lc) :: clpath
    TYPE(ProfileData), SAVE :: psy_profile0
    CALL ProfileStart('rst_read_open', 'r0', psy_profile0)
    IF (numror <= 0) THEN
      IF (lwp) THEN
        WRITE(numout, FMT = *)
        SELECT CASE (jprstlib)
  ...
  SUBROUTINE rst_read
    ...
    TYPE(ProfileData), SAVE :: psy_profile0
    CALL ProfileStart('rst_read', 'r0', psy_profile0)
    CALL rst_read_open
