...
902 IF (ios > 0) CALL ctl_nam(ios, 'namsbc in configuration namelist', lwp)
  ...
  SUBROUTINE sbc(kt)
    ...
    CASE (jp_flx)
      CALL sbc_flx(kt)
    CASE (jp_blk)
      IF (ll_sas) CALL sbc_cpl_rcv(kt, nn_fsbc, nn_ice)
      CALL sbc_blk(kt)
    CASE (jp_purecpl)
      CALL sbc_cpl_rcv(kt, nn_fsbc, nn_ice)
    CASE (jp_none)
      IF (ll_opa) CALL sbc_cpl_rcv(kt, nn_fsbc, nn_ice)
    END SELECT
    ...
    IF (ln_icebergs) THEN
      CALL icb_stp(kt)
      IF (.NOT. ln_passive_mode) CALL lbc_lnk(emp, 'T', 1.)
    END IF
    ...
    IF (kt == nit000) THEN
      IF (ln_rstart .AND. iom_varid(numror, 'utau_b', ldstop = .FALSE.) > 0) THEN
        ...
        END IF
      ELSE
        CALL ProfileStart('sbc', 'r3', psy_profile3)
      ...
      END IF
    END IF
    CALL ProfileStart('sbc', 'r4', psy_profile4)
    IF (lrst_oce) THEN
      IF (lwp) WRITE(numout, FMT = *)
