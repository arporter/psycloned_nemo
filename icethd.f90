MODULE icethd
  ...
  USE sbc_ice, ONLY: qsr_oce, qns_oce, qemp_oce, qsr_ice, qns_ice, dqns_ice, evap_ice, qprec_ice, qevap_ice, qml_ice, qcn_ice, qtr_ice_top
  USE ice1D
  USE icethd_zdf
  USE icethd_dh
  USE icethd_da
  ...
  SUBROUTINE ice_thd(kt)
    ...
    DO jj = 1, jpj
      DO ji = 1, jpi
        ...
        zqld = tmask(ji, jj, 1) * rdt_ice * ((1._wp - at_i_b(ji, jj)) * qsr_oce(ji, jj) * frq_m(ji, jj) + (1._wp - at_i_b(ji, jj)) * qns_oce(ji, jj) + qemp_oce(ji, jj))
    ...
    oa_i(:, :, :) = o_i(:, :, :) * a_i(:, :, :)
    !$ACC END KERNELS
    CALL ProfileStart('ice_thd', 'r4', psy_profile4)
