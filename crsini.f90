MODULE crsini
  ...
  SUBROUTINE crs_init
    ...
    CALL profile_psy_data0 % PreStart('crs_init', 'r0', 0, 0)
...
902 IF (ios > 0) CALL ctl_nam(ios, 'namcrs in configuration namelist', lwp)
    ...
    !$ACC END KERNELS
    CALL profile_psy_data1 % PreStart('crs_init', 'r1', 0, 0)
    ...
    ze3w(:, :, :) = e3w_n(:, :, :)
    !$ACC END KERNELS
    CALL profile_psy_data2 % PreStart('crs_init', 'r2', 0, 0)
    ...
    !$ACC END KERNELS
    CALL profile_psy_data3 % PreStart('crs_init', 'r3', 0, 0)
    ...
    r1_bt_crs(:, :, :) = 0._wp
    !$ACC END KERNELS
    CALL profile_psy_data4 % PreStart('crs_init', 'r4', 0, 0)
