MODULE lbclnk
    ...
    ELSE
       DO jf = 1, ipf
        ...
        ELSE IF (ll_nfd) THEN
           IF (.NOT. znat == 'F')THEN
          !$ACC KERNELS
              ptab(jf) % pt3d(:, 1, :) = zland
          !$ACC END KERNELS
           END IF
          CALL lbc_nfd(ptab, cd_nat(:), psgn(:), ipf)
        ELSE
          !$ACC KERNELS
          IF (.NOT. znat == 'F') ptab(jf) % pt3d(:, 1, :) = zland
  ...
  SUBROUTINE lbc_lnk_2d_multi(pt1, cdna1, psgn1, pt2, cdna2, psgn2, pt3, cdna3, psgn3, pt4, cdna4, psgn4, pt5, cdna5, psgn5, pt6, cdna6, psgn6, pt7, cdna7, psgn7, pt8, cdna8, psgn8, pt9, cdna9, psgn9, cd_mpp, pval)
    ...
    REAL(KIND = wp), OPTIONAL, INTENT(IN   ) :: pval
    INTEGER :: kfld
    TYPE(PTR_2D), ALLOCATABLE :: ptab_ptr(:)
    CHARACTER(LEN = 1), ALLOCATABLE :: cdna_ptr(:)
    REAL(KIND = wp), ALLOCATABLE :: psgn_ptr(:)
!    TYPE(PTR_2D), DIMENSION(9) :: ptab_ptr
!    CHARACTER(LEN = 1), DIMENSION(9) :: cdna_ptr
!    REAL(KIND = wp), DIMENSION(9) :: psgn_ptr
    TYPE(ProfileData), SAVE :: psy_profile0
    ALLOCATE(cdna_ptr(9), psgn_ptr(9))
    CALL ProfileStart('lbc_lnk_2d_multi', 'r0', psy_profile0)
    kfld = 0
    CALL lbc_lnk_2d(pt1, cdna1, psgn1, cd_mpp, pval)
    IF (PRESENT(psgn2)) CALL lbc_lnk_2d(pt2, cdna2, psgn2, cd_mpp, pval)
    IF (PRESENT(psgn3)) CALL lbc_lnk_2d(pt3, cdna3, psgn3, cd_mpp, pval)
    IF (PRESENT(psgn4)) CALL lbc_lnk_2d(pt4, cdna4, psgn4, cd_mpp, pval)
    IF (PRESENT(psgn5)) CALL lbc_lnk_2d(pt5, cdna5, psgn5, cd_mpp, pval)
    IF (PRESENT(psgn6)) CALL lbc_lnk_2d(pt6, cdna6, psgn6, cd_mpp, pval)
    IF (PRESENT(psgn7)) CALL lbc_lnk_2d(pt7, cdna7, psgn7, cd_mpp, pval)
    IF (PRESENT(psgn8)) CALL lbc_lnk_2d(pt8, cdna8, psgn8, cd_mpp, pval)
    IF (PRESENT(psgn9)) CALL lbc_lnk_2d(pt9, cdna9, psgn9, cd_mpp, pval)
...
!!$    CALL lbc_lnk_ptr(ptab_ptr, cdna_ptr, psgn_ptr, kfld, cd_mpp, pval)
  ...
  SUBROUTINE lbc_lnk_3d_multi(pt1, cdna1, psgn1, pt2, cdna2, psgn2, pt3, cdna3, psgn3, pt4, cdna4, psgn4, pt5, cdna5, psgn5, pt6, cdna6, psgn6, pt7, cdna7, psgn7, pt8, cdna8, psgn8, pt9, cdna9, psgn9, cd_mpp, pval)
    USE profile_mod, ONLY: ProfileData, ProfileStart, ProfileEnd
    REAL(KIND = wp), DIMENSION(:, :, :), INTENT(INOUT) :: pt1
    REAL(KIND = wp), DIMENSION(:, :, :), OPTIONAL, INTENT(INOUT) :: pt2, pt3, pt4, pt5, pt6, pt7, pt8, pt9
!    REAL(KIND = wp), DIMENSION(:, :, :), TARGET, INTENT(INOUT) :: pt1
!    REAL(KIND = wp), DIMENSION(:, :, :), OPTIONAL, TARGET, INTENT(INOUT) :: pt2, pt3, pt4, pt5, pt6, pt7, pt8, pt9
    CHARACTER(LEN = 1), INTENT(IN   ) :: cdna1
    CHARACTER(LEN = 1), OPTIONAL, INTENT(IN   ) :: cdna2, cdna3, cdna4, cdna5, cdna6, cdna7, cdna8, cdna9
    ...
    !CHARACTER(LEN = 1), DIMENSION(9) :: cdna_ptr
    !REAL(KIND = wp), DIMENSION(9) :: psgn_ptr
    TYPE(PTR_3D), ALLOCATABLE :: ptab_ptr(:)
    CHARACTER(LEN = 1), ALLOCATABLE :: cdna_ptr(:)
    REAL(KIND = wp), ALLOCATABLE :: psgn_ptr(:)
    TYPE(ProfileData), SAVE :: psy_profile0
    
    ALLOCATE(cdna_ptr(9), psgn_ptr(9), ptab_ptr(0))
...
!!$       CALL lbc_lnk_ptr(ptab_ptr, cdna_ptr, psgn_ptr, kfld, cd_mpp, pval)
    CALL lbc_lnk_3d(pt1, cdna1, psgn1, cd_mpp, pval)
    IF (PRESENT(psgn2)) CALL lbc_lnk_3d(pt2, cdna2, psgn2, cd_mpp, pval)
    IF (PRESENT(psgn3)) CALL lbc_lnk_3d(pt3, cdna3, psgn3, cd_mpp, pval)
    IF (PRESENT(psgn4)) CALL lbc_lnk_3d(pt4, cdna4, psgn4, cd_mpp, pval)
    IF (PRESENT(psgn5)) CALL lbc_lnk_3d(pt5, cdna5, psgn5, cd_mpp, pval)
    IF (PRESENT(psgn6)) CALL lbc_lnk_3d(pt6, cdna6, psgn6, cd_mpp, pval)
    IF (PRESENT(psgn7)) CALL lbc_lnk_3d(pt7, cdna7, psgn7, cd_mpp, pval)
    IF (PRESENT(psgn8)) CALL lbc_lnk_3d(pt8, cdna8, psgn8, cd_mpp, pval)
    IF (PRESENT(psgn9)) CALL lbc_lnk_3d(pt9, cdna9, psgn9, cd_mpp, pval)
    CALL ProfileEnd(psy_profile0)
